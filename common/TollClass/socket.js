...
var WBT = function(obj) {
  ...
  this.url = ENV.getENV()[this.config.url];
  this.timer = null;
  //socket对象
  this.socket;
...
WBT.prototype.handleEvent = function(message) {
  if (message.data !== "连接成功") {
    ...
    //根据不同type处理不同逻辑
    if (this.handleAction[data.event]) {
      this.handleAction[data.event](data.data);
    } else if (!data.event) {
      this.handleAction.order(data.data);
    }
    if (data.event == "conn") {
      _comm.$lStore.set("socketId", data.data);
    }
  }
...
WBT.prototype.Send = function(data) {
  ...
  var timer = setInterval(() => {
    if (this.socket.readyState == 1) {
      if (!_this.config.isId) {
        data.id = _comm.$lStore.get("socketId");
      }
...
WBT.prototype.handleAction = {
  req(data) {
    _comm.$EventListener.fire("TVhistory", data);
  },
  req_list(data) {
    _comm.$EventListener.fire("Reqlist", data);
  },
  req_detail(data) {
    _comm.$EventListener.fire("Reqdatail", data);
  },
  kline(data) {
  ...
  order(data) {
    _comm.$EventListener.fire("order", data);
  }
};
